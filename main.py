...
def create_map(location: tuple) -> object:
    ...
    for i in range(len(local_films)):
        ...
        film_location = local_films.iloc[i]["Coordinates"]
        local_films_layer.add_child(folium.Marker(location=film_location,popup = film_name, icon = folium.Icon()))
...
def find_closest_locations(films, location, year):
    ...
    year_films['Distance'] = year_films['Coordinates'].apply(lambda x: find_distance(location,x))
    for i in range(len(year_films)):
        ...
        elif len(closest_films) == 5 and geo_distance < closest_films["Distance"].values[-1]:
